...
class PrinterPlugin(OctoPrintPlugin):
	"""
	The `PrinterPlugin` mixin allows plugin developers to specify their own printer controllers for
	adding support for potentially any printer-like device to OctoPrint.
	Without one (and only one) printer plugin enabled, OctoPrint will not function properly.

	The OctoPrintPrinter is the default PrinterPlugin.
	"""

	def startup(self, fileManager, analysisQueue, printerProfileManager):
		"""
		Initilizes the plugin for use.
		"""
		pass
...
class MachineComPrintCallbackPlugin(OctoPrintPlugin):
